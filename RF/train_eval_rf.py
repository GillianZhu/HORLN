...
if __name__ == '__main__':
    ...
    params = {"n_estimators": 43, 'min_samples_leaf': 7, 'min_samples_split': 3}
    RFModel = RandomForestClassifier()
    RFModel.set_params(**params)
    RFModel = RFModel.fit(elec_data.cpu().numpy().reshape(data_size_train, -1), flag_data.reshape(-1, 1).squeeze(1).cpu().numpy())

    print("train time:%.2f"%(time.time() - train_start_time))

    x_pred = RFModel.predict(elec_data_test.cpu().numpy().reshape(data_size_test, -1))

    test_start_time = time.time()

    x_pred_probas = RFModel.predict_proba(elec_data_test.cpu().numpy().reshape(data_size_test, -1))
